...
def overwrite_config():
    container_names_out = subprocess.check_output(['sudo', '-u', 'svgaming', 'podman', 'ps', '-a', '--no-trunc', '--size', '--format', '{{.Names}}'])
    container_images_out = subprocess.check_output(['sudo', '-u', 'svgaming', 'podman', 'ps', '-a', '--no-trunc', '--size', '--format', '{{.Image}}'])
...
def main():
    ...
    if os.path.exists("/etc/bpkg.yaml"):
        ...
        print("\033[1m\033[36m>> i: \033[39mThe bpkg config file located at \"/etc/bpkg.yml\" seems to be missing. Generating config...\033[0m")
        overwrite_config()
        print("\033[1m\033[36m>> i: \033[39mdone\033[0m")
        conf = open("/etc/bpkg.yaml", "r+")
    if len(sys.argv) < 2:
